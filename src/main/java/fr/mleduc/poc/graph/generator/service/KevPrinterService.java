...
public class KevPrinterService {
    public String process(final Graph graph) {
        final Stream<String> nodesStream = graph
                .getNodes()
                .stream()
                .map(component -> "add " + component.getName() + ": " + component.getTypeDef().getName());

        final Stream<String> componentsStream = graph
                .getComponents()
                .stream()
                .map(component -> "add " + component.getNode().getName() + "." + component.getName() + ": " + component.getTypeDef().getName());

        final Stream<String> chansStream = graph
                .getChans()
                .stream()
                .map(chan -> {
                    final List<String> lst = new ArrayList<>();
                    lst.add("add " + chan.getName() + ": WSChan");
                    lst.add("set " + chan.getName() + ".host = '" + chan.getSettings().getHost() + "'");
                    lst.add("set " + chan.getName() + ".port = '" + chan.getSettings().getPort() + "'");
                    lst.add("set " + chan.getName() + ".path = '" + chan.getSettings().getPath() + "'");
                    return lst.stream();
                }).flatMap(Function.identity());

        final Stream<String> bindsStream = graph
                .getBinds()
                .stream()
                .map(bind -> {
        ...
        final List<String> res = Stream.of(nodesStream, componentsStream, chansStream, bindsStream).flatMap(Function.identity())
