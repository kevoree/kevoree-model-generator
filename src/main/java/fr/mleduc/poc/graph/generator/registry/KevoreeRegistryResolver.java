...
public class KevoreeRegistryResolver {
	...
	public boolean resolve(final List<TypeFQN> fqns, final ContainerRoot current, final KevoreeFactory factory)
			throws Exception {
		...
		try {
			...
			if (model != null && !model.isEmpty()) {
				final ContainerRoot modelRoot = (ContainerRoot) factory.createJSONLoader().loadModelFromString(model)
						.get(0);
				final Integer[] i = { 0 };
				final StringBuilder builder = new StringBuilder();
				modelRoot.deepVisitReferences(new ModelVisitor() {
					@Override
					public void visit(final KMFContainer kmfContainer, final String s,
							final KMFContainer kmfContainer2) {
